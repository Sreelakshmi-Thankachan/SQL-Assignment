...
SELECT 
    ...
    SUM(CASE
        ...
        ELSE 0
    END) AS null_worlwide_gross_income_count,
    SUM(CASE
...
SELECT 
    month(date_published) AS month_num, COUNT(id) AS movie_count
FROM
    movie
GROUP BY month_num 
ORDER BY month_num;
...
/*
OBSERVATIONS : 
Of the three years 2017, 2018, 2019
- large number of movies released in 2017 (3052)
- less number of movies released in 2019 (2001)
  - large number of movies released in the MONTH March
  - less number of movies released in the MONTH December
*/
...
/*
There are 13 different genre of movies
*/
...
SELECT 
    year, genre, COUNT(movie_id) AS movie_count
FROM
    movie AS m
        INNER JOIN
...
/*
OBSERVATIONS : 
The 'Drama' genre has the large number of movies produced 1078 in the last year (2019)
*/
...
/*
OBSERVATIONS :
Number of movies belong to only one genre is 3289
*/
...
-- Type your code below:
SELECT 
    genre, ROUND(AVG(duration), 3) AS avg_duration
FROM
    movie AS m
        INNER JOIN
    genre AS g ON m.id = g.movie_id
...
/*
OBSERVATIONS :
The Action genre has the highest duration of 112.883 minutes
The Horror genre has the smallest duration of 92.724 minutes
*/
...
WHERE
    year = 2017
        AND MONTH(date_published) = 3 AND total_votes > 1000
GROUP BY genre, country
ORDER BY movie_count DESC
)
select * from usa where country = 'USA'
;
